...
use super::types::{Memory, MemoryQuery, MemorySearchResult, MemoryRelationship, MemoryConfig};
...
impl MemoryStore {
	...
	pub async fn delete_memory(&mut self, memory_id: &str) -> Result<()> {
		...
		self.relationships.retain(|_, rel| {
			rel.source_id != memory_id && rel.target_id != memory_id
		});
	...
	pub async fn search_memories(&self, query: &MemoryQuery) -> Result<Vec<MemorySearchResult>> {
		...
		if let Some(ref query_text) = query.query_text {
			let query_embedding = self.embedding_provider.generate_embedding(query_text).await?;
			...
			for memory in self.memories.values() {
				...
				let memory_embedding = self.embedding_provider.generate_embedding(&memory.get_searchable_text()).await?;
				let similarity = self.calculate_cosine_similarity(&query_embedding, &memory_embedding);
	...
	pub async fn get_memory_relationships(&self, memory_id: &str) -> Result<Vec<MemoryRelationship>> {
		let relationships: Vec<MemoryRelationship> = self.relationships
			.values()
	...
	pub async fn cleanup_old_memories(&mut self) -> Result<usize> {
		if let Some(cleanup_days) = self.config.auto_cleanup_days {
			...
			self.memories.retain(|_, memory| {
				memory.created_at >= cutoff_date || memory.metadata.importance >= self.config.cleanup_min_importance
			});
	...
	fn matches_filters(&self, memory: &Memory, query: &MemoryQuery) -> bool {
		...
		if let Some(ref files) = query.related_files {
			if !files.iter().any(|file| memory.metadata.related_files.contains(file)) {
				return false;
			}
		}
